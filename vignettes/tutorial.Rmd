...
```{r all-together, echo=FALSE, fig.cap="*Plot 1*: Data modelling is messy: which model is the right one?", out.width = '100%'}
...
were run in a new (that is, empty) R session. This in an example from
the `repository` package, see `?repository::iris_model` for details.
...
![*Plot 2*: Objects in R session and artifacts in repository.](graphics/commits.png)
...
Now we will search for a plot stored in a file. `repository` contains
...
cat(sprintf("![Plot stored under repository/artifacts/iris-predictions.png](%s)", path))
```
...
more elaborate example using `repository::london_meters`

```{r attach,include=FALSE}
chronicler:::attach_to_repository(repository::london_meters())
...
library(chronicler)
attach_with_wizard(repository::london_meters())
```
